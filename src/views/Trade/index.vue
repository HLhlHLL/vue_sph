<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="(address, index) in userAddress" :key="index">
        <span class="username" :class="{ selected: address.isDefault === '1' }">{{ address.consignee }}</span>
        <p @click="changeDefaultAdd(address, userAddress)">
      ...
      <div class="detail">
        <h5>商品清单</h5>
        <ul class="list clearFix" v-for="(detail, index) in detailList" :key="index">
          <li>
            <img :src="detail.imgUrl" style="width: 100px" />
          </li>
          <li>
            <p>{{ detail.detailList }}</p>
            <h4>7天无理由退货</h4>
          </li>
          <li>
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{ defaultTradeInfo.fullAddress }}</span>
        收货人：<span>{{ defaultTradeInfo.consignee }}</span>
        <span>{{ defaultTradeInfo.phoneNum }}</span>
      </div>
    </div>
...
export default {
  ...
  methods: {
    changeDefaultAdd(address, userAddress) {
    ...
    async submitOrder() {
      ...
      const data = {
        consignee: this.defaultTradeInfo.consignee,
        consigneeTel: this.defaultTradeInfo.phoneNum,
        deliveryAddress: this.defaultTradeInfo.fullAddress,
        paymentWay: 'ONLINE',
        orderComment: this.msg,
        orderDetailList: this.detailList
      }
      const { data: res } = await this.$http.reqSubmitOrder(tradeNo, data)
      if (res.code === 200) {
